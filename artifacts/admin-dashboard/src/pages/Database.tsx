  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-[10px] font-black uppercase tracking-widest mb-0.5" style={{ color: "rgba(11,33,71,0.40)" }}>System · registry_production_v4</p>
          <h2 className="text-[26px] font-black leading-tight" style={{ color: "#0b2147" }}>Database Admin</h2>
          <p className="text-[12px] mt-0.5" style={{ color: "rgba(11,33,71,0.45)" }}>Browse and edit core database tables directly</p>
        </div>
        <button onClick={loadTables} disabled={tablesLoading}
          className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold transition-all hover:bg-slate-100 disabled:opacity-50"
      ...
      <div className="flex gap-5">
        ...
        <div className="w-64 shrink-0">
          <div className="bg-white rounded-2xl overflow-hidden" style={{ border: "1px solid rgba(11,33,71,0.08)", boxShadow: "0 1px 6px rgba(11,33,71,0.05)" }}>
            {/* Schema header */}
            <div className="px-4 py-3 border-b" style={{ borderColor: "rgba(11,33,71,0.07)" }}>
              <div className="flex items-center justify-between mb-1">
                <span className="text-[10px] font-black uppercase tracking-widest" style={{ color: "rgba(11,33,71,0.40)" }}>Schema: Public</span>
                <span className="px-2 py-0.5 rounded-full text-[9px] font-bold" style={{ background: "rgba(34,197,94,0.12)", color: "#16a34a" }}>CONNECTED</span>
              </div>
              <div className="relative mt-2">
                <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 h-3 w-3" style={{ color: "rgba(11,33,71,0.35)" }} />
                <input type="text" placeholder="Filter tables..." className="w-full pl-7 pr-2 py-1.5 text-[11px] rounded-lg focus:outline-none focus:ring-1" style={{ background: "rgba(11,33,71,0.04)", border: "1px solid rgba(11,33,71,0.08)", color: "#0b2147" }} readOnly />
              </div>
            </div>
            {/* Tables count */}
            <div className="px-4 py-2 border-b" style={{ borderColor: "rgba(11,33,71,0.06)" }}>
              <span className="text-[10px] font-bold uppercase tracking-widest" style={{ color: "rgba(11,33,71,0.40)" }}>Tables ({tables.length})</span>
            </div>
            {tablesLoading ? (
              ...
              <div className="p-2">
                {tables.map(t => (
                  <button
                    ...
                    className="w-full text-left px-3 py-3 rounded-xl transition-all duration-150 mb-1 group"
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-2 min-w-0">
                        <Table2 className="h-3.5 w-3.5 shrink-0" style={{ color: selectedTable === t.name ? "rgba(255,255,255,0.55)" : "rgba(11,33,71,0.35)" }} />
                        <span className="font-semibold text-sm truncate">{TABLE_LABELS[t.name] ?? t.name}</span>
                      </div>
                      <span className="text-[10px] font-mono px-1.5 py-0.5 rounded-lg shrink-0"
                        style={selectedTable === t.name
                          ? { background: "rgba(255,255,255,0.12)", color: "rgba(255,255,255,0.70)" }
                          : { background: "rgba(11,33,71,0.06)", color: "rgba(11,33,71,0.50)" }}>
          ...
          {/* Warning box */}
          <div className="mt-3 p-3 rounded-xl" style={{ background: "rgba(245,158,11,0.08)", border: "1px solid rgba(245,158,11,0.20)" }}>
            <div className="flex items-start gap-2">
              <AlertTriangle className="h-3.5 w-3.5 mt-0.5 shrink-0" style={{ color: "#d97706" }} />
              <p className="text-[10px] leading-relaxed" style={{ color: "#92400e" }}>Editing data directly bypasses validation. Changes are immediate and permanent.</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 min-w-0">
          {!selectedTable ? (
            <div className="bg-white rounded-2xl p-10 text-center flex flex-col items-center" style={{ border: "1px solid rgba(11,33,71,0.08)", boxShadow: "0 1px 6px rgba(11,33,71,0.05)" }}>
              {/* Big grid icon */}
              <div className="h-20 w-20 rounded-2xl flex items-center justify-center mb-6" style={{ background: "rgba(11,33,71,0.06)" }}>
                <Table2 className="h-10 w-10" style={{ color: "rgba(11,33,71,0.25)" }} />
              </div>
              <h3 className="text-2xl font-black mb-2" style={{ color: "#0b2147" }}>SELECT A TABLE</h3>
              <p className="text-sm mb-8 max-w-sm leading-relaxed" style={{ color: "rgba(11,33,71,0.45)" }}>
                Browse the registry schema on the left to view, edit, or export raw database records. Use caution when performing write operations.
              </p>
              {/* Stat cards */}
              <div className="grid grid-cols-2 gap-4 w-full max-w-sm mb-8">
                <div className="rounded-xl p-4 text-left" style={{ background: "rgba(11,33,71,0.04)", border: "1px solid rgba(11,33,71,0.07)" }}>
                  <div className="flex items-center gap-2 mb-2">
                    <DbIcon className="h-4 w-4" style={{ color: "rgba(11,33,71,0.35)" }} />
                    <p className="text-[10px] font-bold uppercase tracking-wider" style={{ color: "rgba(11,33,71,0.45)" }}>Total Records</p>
                  </div>
                  <p className="text-xl font-black" style={{ color: "#0b2147" }}>
                    {tables.reduce((s, t) => s + t.rowCount, 0).toLocaleString()}
                  </p>
                </div>
                <div className="rounded-xl p-4 text-left" style={{ background: "rgba(11,33,71,0.04)", border: "1px solid rgba(11,33,71,0.07)" }}>
                  <div className="flex items-center gap-2 mb-2">
                    <Table2 className="h-4 w-4" style={{ color: "rgba(11,33,71,0.35)" }} />
                    <p className="text-[10px] font-bold uppercase tracking-wider" style={{ color: "rgba(11,33,71,0.45)" }}>Relations</p>
                  </div>
                  <p className="text-xl font-black" style={{ color: "#0b2147" }}>{tables.length} active</p>
                </div>
              </div>
              <button
                onClick={loadTables}
                className="flex items-center gap-2 px-6 py-2.5 rounded-xl text-white text-sm font-bold transition-all hover:-translate-y-0.5 hover:shadow-lg"
